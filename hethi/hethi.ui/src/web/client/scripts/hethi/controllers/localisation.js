/**
 * Created by saifudheen on 1/5/2016.
 */
hethi.factory('localize', [
        ...
        i18nDirective = {
            ...
            updateText: function(ele, input, placeholder) {
                ...
                result = void 0;
                 if (input.length >= 1) {
                    result = localize.getLocalizedString(input);
                     return result? ele.attr('src', result):  ele.attr('src', input);

                }
            },
            link: function(scope, ele, attrs) {

                //attrs['src']=i18nDirective.updateText(ele, attrs.i18nSrc, attrs.placeholder);;

                scope.$on('localizeResourcesUpdated', function() {
                    return i18nDirective.updateText(ele, attrs.i18nSrc, attrs.placeholder);
                });
                return attrs.$observe('i18nSrc', function(value) {
    ...
    '$scope', 'localize', function($scope, localize) {
        ...
        $scope.setLang = function(lang) {
            ...
            localStorage.setItem("hethi-language",lang);
            return $scope.lang = lang;
        };

        if( localStorage.getItem("hethi-language")!=undefined) {
            $scope.lang = localStorage.getItem("hethi-language");
