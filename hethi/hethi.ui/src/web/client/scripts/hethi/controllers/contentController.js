...
hethi.controller('contentController', ['$http','$scope','$filter','$location','$rootScope', function ($http,$scope,$filter,$location,$rootScope){
    ...
    $scope.loadIxsd_data=function(index){
        ...
        $http({
            method: 'POST',
            url: $rootScope.spring_rest_service+'/search_ixsd_content',
            dataType:'jsonp',
            data:input
            ...
            if($scope.POtab==1){
                ...
                for(var key in data[last]){
                    $scope.POLineItem.push({key:key,value:data[last][key]})
                }
            }
            else if($scope.POtab==2)
