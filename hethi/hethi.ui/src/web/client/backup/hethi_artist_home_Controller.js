        ...
        if ($scope.masterFormFields.mxsd.form.header.group.length != 0) {
            ...
            $scope.masterFormFields.mxsd.form.header.group.forEach(function (row) {
                if(row.field.length!=0) {
                    ...
                    {
                       coords.push({
                            x: (900/$scope.formSize.Height * rows.label.position.left),
                            y: ( 876/$scope.formSize.width * rows.label.position.top),
                            width: (876/$scope.formSize.width * rows.label.position.width),
                            height: (900/$scope.formSize.Height  * rows.label.position.height)
                        });
                        coords.push({
                            x: (900/$scope.formSize.Height * rows.data.position.left),
                            y: (876/$scope.formSize.width * rows.data.position.top),
                            width: (876/$scope.formSize.width * rows.data.position.width),
                            height: (900/$scope.formSize.Height * rows.data.position.height)
                        });
                    })
                }
            });
            ...
            $scope.masterFormFields.mxsd.form.body.group.forEach(function (row) {
                ...
                if(row.fieldlist.field.length!=0){
                    row.fieldlist.field.forEach(function (rows) {
                        coords.push({
                            x: (900 / $scope.formSize.width * rows.label.position.left),
                            y: (876 / $scope.formSize.width * rows.label.position.top),
                            width: (876 / $scope.formSize.width * rows.label.position.width),
                            height: (900 / $scope.formSize.width * rows.label.position.height)
                        });
                        coords.push({
                            x: (900 / $scope.formSize.width * rows.data.position.left),
                            y: (876 / $scope.formSize.width * rows.data.position.top),
                            width: (876 / $scope.formSize.width * rows.data.position.width),
                            height: (900 / $scope.formSize.width * rows.data.position.height)
                        });
                    });
                }
            })
